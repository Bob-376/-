...
import { Bot, User, Hash, Copy, Check, Clock } from 'lucide-react';
    ...
    <div className={`flex w-full mb-6 ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className={`group flex max-w-[85%] md:max-w-[75%] ${isUser ? 'flex-row-reverse' : 'flex-row'} items-start gap-3`}>
        ...
        <div className="flex flex-col">
          ...
          {!isThinking && (
            <div className={`
              mt-1 px-1 flex items-center gap-1 text-[9px] md:text-[10px] uppercase tracking-tighter transition-opacity duration-300 opacity-0 group-hover:opacity-50 font-sans
              ${isUser ? 'justify-end text-gray-500' : 'justify-start text-himalaya-red'}
            `}>
              <Clock size={10} />
